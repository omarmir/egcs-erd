...
function onOpen() {
  ...
  const menu = ui.createMenu("Export Database")
  ...
  menu.addItem(
    "Generate DBML (Current Sheet, Tables Only)",
    "exportCurrentSheetTablesOnlyDBML",
  )
  menu.addItem("Generate PGML (All Sheets)", "exportPGML")
  menu.addItem("Generate PGML (Current Sheet)", "exportCurrentSheetPGML")
  menu.addItem(
    "Generate PGML (Current Sheet, Tables Only)",
    "exportCurrentSheetTablesOnlyPGML",
  )
...
function buildColumnMap(headerRow) {
  ...
  headerRow.forEach((col, index) => {
    ...
    if (name.includes("logical")) map.logicalName = index
    if (name.includes("optional")) map.optional = index
    if (name.includes("field type")) map.fieldType = index
    if (name.includes("relation")) map.relation = index
    if (name.includes("description")) map.description = index
  })

  return map
}

function isIndexesOrChecksSectionStart(firstCell) {
  const s = String(firstCell || "")
    .trim()
    .toLowerCase()
  return s === "index name" || s === "check name" || s === "checks"
}
...
function parseTableSheet(sheet, schemaName, areaId, areaColor) {
  ...
  let layoutIndex = 0
  let columnMap = null
  let skipNonFieldSections = false // true when inside Indexes/Checks blocks
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (
      ...
      firstCell &&
      !isHeaderRow(row) &&
      !isIndexesOrChecksSectionStart(firstCell)
    ) {
      ...
      const posY =
        Math.floor(layoutIndex / CONFIG.tablesPerRow) * CONFIG.spacingY * 8
      layoutIndex++
      ...
      tableMap.set(tableKey(schemaName, tableName), {
        id: tableId,
        schema: schemaName,
        fields: new Map(),
      })

      skipNonFieldSections = false // ✅ RESET for new table
      continue
    }

    if (!currentTable || !columnMap) continue

    // ✅ Enter “skip mode” when Indexes/Checks blocks start
    if (isIndexesOrChecksSectionStart(firstCell)) {
      skipNonFieldSections = true
      continue
    }

    if (skipNonFieldSections) continue

    if (!firstCell) continue
    if (isHeaderRow(row)) continue

    // --- SAFE COLUMN ACCESS ---
    const fieldName = row[columnMap.logicalName] || ""
    const optionalFlag = String(row[columnMap.optional] || "").toUpperCase()
    const fieldTypeRaw = String(row[columnMap.fieldType] || "")
    const relationRaw = String(row[columnMap.relation] || "")
    ...
    tableMap
      .get(tableKey(schemaName, currentTable.name))
      .fields.set(fieldName, fieldId)

    // --- FK Processing ---
    if (relationRaw.toLowerCase().includes("foreignkey")) {
 ...
 */
function processRelationships2() {
  tables.forEach((table) => {
    ...
    table.rawRelationships.forEach((rel) => {
      ...
      if (targetData) {
        ...
        console.warn(
          `Foreign key target not found: ${table.schema}.${rel.targetTableName}`,
        )
      }
    })
...
function processRelationships() {
  tables.forEach((table) => {
    ...
    table.rawRelationships.forEach((rel) => {
      ...
      // 1️⃣ Try same schema first
      const sameSchemaKey = tableKey(table.schema, rel.targetTableName)
      targetData = tableMap.get(sameSchemaKey)

      // 2️⃣ If not found, search across ALL schemas
      if (!targetData) {
        for (const [key, value] of tableMap.entries()) {
          const [, tableName] = key.split(".")
          if (tableName === rel.targetTableName) {
            targetData = value
            break
          }
        }
      ...
      if (!targetData) {
        console.warn(
          `Foreign key target not found anywhere: ${rel.targetTableName}`,
        )
        return
      }
      ...
      if (!targetFieldId) {
        console.warn(
          `Target field not found: ${rel.targetTableName}.${rel.targetFieldName}`,
        )
        return
      }
