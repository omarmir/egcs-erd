...
function parseTableSheet(sheet, schemaName, areaId, areaColor) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    // --- New Table Detection ---
    if (bg !== '#ffffff' && firstCell && !isHeaderRow(row) && !isIndexesOrChecksSectionStart(firstCell)) {
    ...
    tableMap.get(tableKey(schemaName, currentTable.name))
...
function processRelationships() {
  tables.forEach(table => {
    ...
    table.rawRelationships.forEach(rel => {
      ...
      if (!targetData) {
        console.warn(`Foreign key target not found anywhere: ${rel.targetTableName}`);
        return;
      }
      ...
      if (!targetFieldId) {
        console.warn(`Target field not found: ${rel.targetTableName}.${rel.targetFieldName}`);
        return;
      }
